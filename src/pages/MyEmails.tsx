          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Track your email template storage and usage limits.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span>{savedEmails.length} / 50</span>
                  </div>
                  <div className="w-full bg-secondary rounded-full h-2">
                    <div 
                      className="bg-primary h-2 rounded-full" 
                      style={{ width: `${Math.min((savedEmails.length / 50) * 100, 100)}%` }}
                    ></div>
                  </div>
                </div>
              </CardContent>
            ...
            <div className="bg-background rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden">
              <div className="flex items-center justify-between p-6 border-b">
                <h2 className="text-2xl font-bold">
                  {savedEmails.find(email => email.id === expandedEmailId)?.subject}
                </h2>
                <button
              ...
              <div className="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-200px)]">
                ...
                <div className="bg-muted rounded-lg p-6">
                  ...
                  <pre className="text-sm overflow-x-auto whitespace-pre-wrap break-words bg-background p-4 rounded border max-h-64 overflow-y-auto">
                    {savedEmails.find(email => email.id === expandedEmailId)?.mjml}
                  </pre>
                </div>
                ...
                <div className="flex space-x-4">
                  <GradientButton
                    ...
                    onClick={() => {
                      if (expandedEmailId) {
                        handleEditEmail(expandedEmailId);
                      }
                    }}
                  ...
                  <GradientButton
                    ...
                    onClick={() => {
                      const email = savedEmails.find(email => email.id === expandedEmailId);
                      if (email) handleCopyMJML(email.mjml);
                    }}
                    className="flex-1"
                  >
                    Copy MJML
