...
export const Step5Results: React.FC<Step5ResultsProps> = ({
  ...
  const handleSaveEmail = (email: any) => {
    // Get existing saved emails from localStorage
    const existingSavedEmails = JSON.parse(localStorage.getItem('savedEmails') || '[]');
    
    // Create a new email object with timestamp
    const emailToSave = {
    ...
    // Update local state
    setSavedEmails(prev => [...prev, email.id]);
              ...
              <div className="bg-background rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden">
                <div className="flex items-center justify-between p-6 border-b">
                  <h2 className="text-2xl font-bold">
                    {formData.generatedEmails.find(email => email.id === expandedEmailId)?.subject}
                  </h2>
                  <button
                ...
                <div className="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-200px)]">
                  ...
                  <div className="bg-muted rounded-lg p-6">
                    ...
                    <div className="bg-background rounded border min-h-[300px] flex items-center justify-center">
                      <div className="text-muted-foreground">
                        MJML Email Render Preview
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-muted rounded-lg p-6">
                    ...
                    <pre className="text-sm overflow-x-auto whitespace-pre-wrap break-words bg-background p-4 rounded border max-h-64 overflow-y-auto">
                      {formData.generatedEmails.find(email => email.id === expandedEmailId)?.mjml}
                    </pre>
                  </div>
                  ...
                  <div className="flex space-x-4">
                    <GradientButton
                      ...
                      onClick={() => {
                        const email = formData.generatedEmails.find(email => email.id === expandedEmailId);
                        if (email) handleCopyMJML(email.mjml, email.id);
                      }}
                      className="flex-1"
                    >
